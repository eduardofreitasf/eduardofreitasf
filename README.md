
# Hi there, I'm Eduardo 👋 [![Linkedin Badge](https://img.shields.io/badge/-LinkedIn-blue?style=flat&logo=Linkedin&logoColor=white)](https://www.linkedin.com/in/eduardoffernandes/)

> Building elegant solutions at the intersection of algorithms, systems design, and clean code.

### 🚀 About Me
My name is **Eduardo Freitas Fernandes** and I'm a **Software Engineering** student at [**Minho University**](https://www.uminho.pt), where I split my time between high-level abstractions and low-level hardware. I enjoy bridging the gap between elegant algorithms and system-level performance.

- 🎓 **Education:** Studying Software Engineering @ [UMinho](https://www.uminho.pt/)
- 🧔 **Role**: Member of the Projects Department @ [EPIC Júnior](https://epicje.pt/)
- 🐧 **Low-Level Enthusiast:** Deeply interested in **Linux internals** and **OS development**.
- 🧩 **Algorithm Fanatic:** Love exploring **Functional Programming**, complex **Data Structures**, and **Big-O Optimization**.
- 🔐 **Security & Comms:** Exploring network security and the fundamentals of digital communication.
- 🔭 I’m currently working on **[Endless](https://github.com/eduardofreitasf/Endless)**
- 🌱 I’m currently learning **Web Development and Systems Modeling**.
- 🌍 Check out my portfolio: **[Portfolio](https://eduardofreitasf.github.io/portfolio/)**

---

### 🧰 My Toolbox

<p align="left">
  <a href="https://skillicons.dev">
    <img src="https://skillicons.dev/icons?i=linux,git,githubactions,docker,cpp,cs,go,py,java,postgres,mysql,ts,vue,react,figma" />
  </a>
</p>

---

### 🌟 Featured Projects

| Project                  | Description                           | Tools      | Link                                                            |
| :------------------------| :-------------------------------------| :--------- | :---------------------------------------------------------------|
| **MechanicPro**          | Management system for a mechanic shop | React Rust | [Repo](https://github.com/eduardofreitasf/MechanicPro)          |
| **IronStruct**           | Library of generic data structures    | C          | [Repo](https://github.com/eduardofreitasf/IronStruct)           |
| **Endless**              | Functional Algorithms in Haskell      | Haskell    | [Repo](https://github.com/eduardofreitasf/Endless)              |
| **MemAlloc**             | Memory allocation library             | C          | [Repo](https://github.com/eduardofreitasf/MemAlloc)             |
| **CBuild**               | Build system for C projects           | Python     | [Repo](https://github.com/eduardofreitasf/CBuild)               |
| **Beat The Deck**        | Beat the deck game                    | VueJS      | [Repo](https://github.com/eduardofreitasf/beat_the_deck)        |
| **Infinite Tic Tac Toe** | Tic Tac Toe with infinite gameplay    | React      | [Repo](https://github.com/eduardofreitasf/infinite_tic_tac_toe) |

---

### 📊 GitHub Stats

<div align="center">
  <p align="center">
    <img height="180em" src="https://github-readme-stats-fast.vercel.app/api?username=eduardofreitasf&show_icons=true&theme=radical&include_all_commits=false&count_private=true" />
    <img height="180em" src="https://github-readme-stats-fast.vercel.app/api/top-langs/?username=eduardofreitasf&layout=compact&theme=radical" />
  </p>
  <img height="180em" src="https://github-readme-stats-fast.vercel.app/api/streak?username=eduardofreitasf&theme=radical" />
</div>
